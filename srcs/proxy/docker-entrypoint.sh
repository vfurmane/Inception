...
mkdir -p /etc/nginx/portfolio.$DOMAIN_NAME
cd /etc/nginx/portfolio.$DOMAIN_NAME
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out /etc/nginx/portfolio.$DOMAIN_NAME/server.key
chmod 400 server.key
openssl req -new -subj "/C=FR/ST=Paris/L=Paris/O=Dis/CN=portfolio.$DOMAIN_NAME" -sha256 -key server.key -out /etc/nginx/portfolio.$DOMAIN_NAME/server.csr
openssl x509 -req -days 1095 -in server.csr -signkey /etc/nginx/portfolio.$DOMAIN_NAME/server.key -out /etc/nginx/portfolio.$DOMAIN_NAME/server.crt;
